import { select, spawn, take, call, put } from '@redux-saga/core/effects';
...
export function* watchTickersSortingSaga() {
  while (true) {
    const action: { payload: { sortBy: TickerListSorting } } = yield take(
      ActionId.SORT_TICKERS_START,
    );
